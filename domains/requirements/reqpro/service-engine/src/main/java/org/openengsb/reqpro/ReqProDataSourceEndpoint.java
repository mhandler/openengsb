 ...
 *                         description="reqpro requirement management concept source endpoint"
 */
public class ReqProDataSourceEndpoint extends DataSourceEndpoint<ReqProProvider> {

    private ConceptSource source;

    @Override
    protected ReqProProvider getImplementation(ContextHelper contextHelper, MessageProperties msgProperties) {
        return new ReqProProvider();
    }
    ...
    private void init() {
        ...
        source.setProvidedConcepts(Arrays.asList(new ConceptKey[] { new ConceptKey("requirement", "1.0.0") }));
    }
